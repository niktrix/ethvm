...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    created() {
      ...
      this.$socket.emit('getBalance', { address: this.address }, (err, result) => {
        if (!err && result) {
      ...
      this.$socket.emit('getTokenBalance', {address: this.address}, (err, result) => {
        if (result.result !== '0x') {
      ...
      this.$socket.emit('getTotalTxs', {address: this.address}, (err, result) => {
        this.account.totalTxs = result
      })
      /*Getting USD Values: */
      this.$socket.emit('getTokenToUSD', {tokens: []}, (err, result) => {
        this.account.ethusd = result[0][1]
        this.usdValue.ETH.value = result[0][1]
      })
      /*Getting Address Transactions: */
      this.$socket.emit('getTxs', {address: this.address}, (err, result) => {
        const txs = []
